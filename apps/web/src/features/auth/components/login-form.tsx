...
import { z } from "zod";
import { type LoginMutationError, useLogin } from "@/api/api";
...
import { getOAuth2AuthorizationUrl } from "@/utils/auth.ts";
	...
	const { mutate: submitLogin, isPending } = useLogin({
		mutation: {
			onSuccess: async (_, variables) => {
				handleSignIn();
				await navigate({ to: "/" });
				toast.success(`Welcome ${variables.data.email}`, {
					duration: 3000,
				});
			},
			onError: (error: LoginMutationError) => {
				...
				if (error.isAxiosError) {
					...
					const errorDetails = response && (response.data as ProblemDetail);
					errorMessage = errorDetails?.detail ?? error.message ?? errorMessage;
				}
				...
				<div className="grid gap-6">
					...
					<div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
						<Button
							...
							onClick={() => initiateOAuth2Login("discord")}
						>
							<svg
